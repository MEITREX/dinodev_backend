...
RUN gradle clean build -x test -x generateApiDocs || return 0

# Now copy the actual app code and build it
COPY . /workspace/app
RUN gradle clean build -x test -x generateApiDocs --info
...
ENTRYPOINT ["java","-cp","app:app/lib/*","de.unistuttgart.iste.meitrex.scrumgame.ScrumGameServiceApplication"]